<template>
    <div class="recommend">
        <recommmend-item v-for="(item,index) in recommendData.list"
                         :itemdata="item" :key="index"></recommmend-item>
    </div>
</template>
      ...
      methods:{
        getdata(){//请求数据的方法
          if (this.goods_style !== null) {
            getRecommendData(this.goods_style.type,this.goods_style.style).then(res => {
